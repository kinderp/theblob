...
fn fixture_component() -> Vec<u8> {
    let path = Path::new(env!("CARGO_MANIFEST_DIR"))
        .join("fixtures/fs-probe/target/wasm32-wasip2/release/blob_wasi_fs_probe.wasm");
    fs::read(&path).unwrap_or_else(|error| {
        panic!(
            "missing WASIp2 fixture at {}: {error}; build fixtures/fs-probe for target wasm32-wasip2 first",
            path.display()
        )
    })
}
